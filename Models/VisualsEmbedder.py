# -*- coding: utf-8 -*-
"""VisualsEmbedder.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1ufUspEmvZUlNLOLsXdBP63pJZ2tgyotq
...
# Check whether the absolute paths are correct 
if SDK_PATH is None:
...
print("=" * 80)
print(dataset[visual_field].keys())
...
for segment in identifiers[:test_index]:
    ...
    _visuals = dataset[visual_field][segment]['features']

    # remove nan values
    label = np.nan_to_num(label)
    _visuals = np.nan_to_num(_visuals)

    print("Visuals data shape:", _visuals.shape, "Labels:", label.shape, "Segment ID:", segment)

    visuals = np.asarray(_visuals)

    # z-normalization per instance and remove nan/infs
    visuals = np.nan_to_num((visuals - visuals.mean(0, keepdims=True)) / (EPS + np.std(visuals, axis=0, keepdims=True)))

    if segment in train_split:
        ###train.append(((words, visual, acoustic), label, segment))
        train.append([visuals, label, segment])
    elif segment in dev_split:
        ###dev.append(((words, visual, acoustic), label, segment))
        dev.append([visuals, label, segment])
    elif segment in test_split:
        ###test.append(((words, visual, acoustic), label, segment))
        test.append([visuals, label, segment])
    else:
...
def multi_collate(batch):
    ...
    visuals = pad_sequence([torch.FloatTensor(sample[0]) for sample in batch])
    
    # lengths might be useful later for attention
    lengths = torch.LongTensor([sample[0].shape[0] for sample in batch])

    return visuals, labels, lengths
...
torch.save(model.state_dict(), LOADER_PATH + 'visual_embedder')

"""## Train the model"""

model = torch.load(LOADER_PATH + 'visual_embedder')
